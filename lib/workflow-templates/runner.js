...
export async function runWorkflow(input) {
    ...
    if (input.previous?.pendingRequest || input.previous?.report.requestUncertain) {
    ...
    while (c.stream < streamCount) {
        ...
        // Persist the in-flight request so a crash cannot silently replay a paid read.
        c.pendingRequest = true;
        c.report.requests += 1;
        await save();
        let page;
        try {
            page = await input.transport.request(path);
        }
        catch (error) {
            if (!(error instanceof WorkflowRequestError) || error.uncertain) {
                c.report.requestUncertain = true;
            ...
            return finish("request_failed", error.message);
        }
        if (!Number.isSafeInteger(page.charged) || page.charged < 0) {
            c.report.requestUncertain = true;
            return finish("uncertain_request", "Could not verify the API charge. Check usage before running again.");
        }
        c.pendingRequest = false;
